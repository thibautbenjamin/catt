...
module Translate : sig
  ...
  let register name env sigma body value =
    Io.debug "calling register with %s" name;
    let sigma, body = Typing.solve_evars env sigma body in
    let body = Evarutil.nf_evar sigma body in
    (* Ugly workaround: Coq presents us to define two globals with the same
     * name, and we would do that to translate the named term associated to a
     * coherence (but it's ok because same definition). We use the name here to
     * disambiguate for now. Needs some changes in the kernel to be handled
     * properly *)
    match Coqlib.lib_ref_opt ("catt_" ^ name) with
    | Some gr ->
        let env = Global.env () in
        let sigma, econstr = Evd.fresh_global env sigma gr in
        (env, sigma, econstr)
    | None ->
        Io.debug "Declaring info";
        let info = Declare.Info.make () in
        let cinfo =
          Declare.CInfo.make ~name:Id.(of_string ("catt_" ^ name)) ~typ:None ()
        in
        Io.debug "Making a globref";
        let gr =
          Declare.declare_definition ~info ~cinfo ~opaque:false ~body sigma
        in
        Io.debug "registering term %s" ("catt_" ^ name);
  ...
  and coh_to_lambda env sigma obj_type eq_type refl coh =
    ...
    | None ->
        ...
        in
        register name env sigma body value
  ...
  and tm_to_lambda env sigma obj_type eq_type refl tm =
    ...
    | Some res -> res
    | None ->
        Io.debug "generating lambda term for term %s" (Tm.name tm);
        let name = clean_name (Tm.name tm) in
        let ctx = Tm.ctx tm in
        let tm = Tm.develop tm in
        let env, sigma, tm =
          tm_to_econstr env sigma obj_type eq_type refl ctx tm
        in
        Io.debug "building body";
        let sigma, body =
