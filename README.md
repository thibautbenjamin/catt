...
### Examples
`catt` is a theory of unbiased ω-categories, which means that one can declare composite of any arity, for instance the binary and ternary composite can be defined as follows:
```
...
```
`catt` is a theory of higher categories, so one can define higher dimensional compositions, such as for instance on 2-cells, the vertical composition, the horizontal compositions and the left and right whiskerings
```
...
```
Coherences can also be combined together to form composite terms that one can define. For instance, here is a biased ternary composite built from combining binary composites:
```
...
```
For instance, the unitor using explicit substitutions can be defined as follows
```
set explicit_substitutions = t
...
```
The same name `comp` is used for the binary composition and the ternary one.
...
### Opposites
The language of `catt` is invariant under the operation of formally reverting all the directions of the cells in a given dimension. We call this operation an opposite, and it is provided by the following syntax
```
op { [NUMBER] } (TERM)
```
...
```
In this case, the chosen inverse is the associator going the opposite direction. There are more complex cases, where one has for instance a composite of associator, in which case the chosen inverse will be a composite of the inverse associators in the opposite order.
...
Given a coherence or a term and an upwards closed subset of variables of codimension 1 in it, `catt` can compute the naturality of this term with respect to that set of variables. This is again indicated with square bracket around the corresponding arguments of the applciation. In order to mark implicit arguements, we provide the construct `@`, which when given before a name indicates that the substitution in this case is explicit. This for instance lets us define the square composite as follows:
```
...
let trcomp (x : *) (y : *) (z : *) (y' : *) (z ' : *)
           ...
           = @comp _ [_] [a] [_] [b]
```
An formal account of this principle is in preparation.
...
## Coq plugin
`catt` also provide a plugin for the proof assistant `coq` (packaged separately, called `coq-catt-plugin`), that allows to export any defined term in `catt` into a function computing higher identity witnesses in `coq`. Once the plugin is install write the following to load it at the head of a `coq` file
```
...
```
where `[NAMES]` is a list of names of `catt` terms and `[PATH]` is a file containing the definition of those `catt` terms. Upon evaluation of this line `coq` will call `catt` on the given file and export the terms with the indeicated names into `coq` terms, were the names will by given as `catt_[NAME]`. For one can write the following:
```
