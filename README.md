Artifacts for Higher Eckmann-Hilton arguments in type theory
============================================================

# Proof artifacts

The proof artifacts produced by running catt to the Eckmann-Hilton terms H^n_{k,l}, are located in the  the directory `artifacts/`. This directory also contain a subdirectory named `source-files` containing the catt source code that we used to produce these artifacts.
...
## Benchmarks and helper script
The directory `benchmark/` contains a subset of source files that we have used to produce our proof artifacts. It also contains a script `benchmark.sh` that executes on all files in the subdirectory and stores the result in a subdirectory `results/`. Since the computations are heavy, these subdirectories only contain the sources for a subset of the artifacts we have produced. The complete set of source files can be found in the directory `artifacts/`.
