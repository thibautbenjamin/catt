...
module Make (Theory : Theory.S) = struct
  ...
  module Codim1 = struct
    ...
    let right_filler n =
      Display_maps.var_apply_sub (left_filler n) (right_incl n)
    ...
    let rec ctx n m k =
      if n > m then
        ...
        | 1 ->
            ...
            in
            let rf =
              Display_maps.var_apply_sub (Cylinder.filler n)
                (Codim1.right_incl n)
            in
        ...
        | _ ->
            ...
            let ctx =
              Functorialisation.ctx ctx [ llb; lub; lf; rlb; rub; rf ]
            in
            let names = Unchecked.db_level_sub_inv ctx in
            let ctx, _, _ = Unchecked.db_levels ctx in
            let rename x = Display_maps.var_apply_sub x names in
        ...
        | n when n = 2 ->
            ...
            let char v =
              Unchecked.sub_ps_to_sub
                ((Var v, true)
                :: Unchecked.ty_to_sub_ps (fst (List.assoc v ctx)))
            in
            let ctx, upper_incl =
