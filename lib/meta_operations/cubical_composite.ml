...
module Make (Theory : Theory.S) = struct
  ...
  module LinearComp = struct
    ...
    let arity ps =
      ...
      let rec aux ps i =
        ...
        | _, _ ->
            Error.fatal
              "cubical composite must be on suspended linear composite"
      in
      aux ps (d - 1)
    ...
    let sub_assc_i i arity l =
      let rec sub k =
        ...
        | k when k = 0 -> [ (tdb 0, false) ]
        | k when k < i + 1 ->
            (tdb k, true) :: (tdb (k - 1), false) :: sub (k - 2)
        | k when k = i + 1 ->
            List.append
  ...
  let biasor_sub_intch_tgt ps bdry_f i1 i2 d =
    ...
    let prod_to_src =
      Unchecked.pullback_up (d - 1) bdry_f ps_red i1 right_leg
    in
    Unchecked.sub_ps_apply_sub_ps red_sub_prod prod_to_src
 ...
 *)
  ...
  let intermediate_ps ps l d =
    ...
    if l_d0 = [] then (ps, l, Unchecked.(sub_ps_to_sub (identity_ps ps)))
      ...
      let ps_f = PS.(mk (Ctx.check ps_f_c)) in
