...
module Make (Theory : Theory.S) = struct
  ...
  module Telescope = Telescope.Make (Theory)
  ...
  let group_vertically ps t src_t tgt_t =
    let coh_unbiased =
      Coh.check_noninv ps src_t tgt_t ("unbiased_comp", 0, [])
    in
  ...
  and compute_witness_comp c s ~ps ~d ~sub_base ~u ~v =
    ...
    in
    let sub_total =
      (m3, true) :: (tgt_m3, false) :: (m2, true) :: (src_m3, false)
