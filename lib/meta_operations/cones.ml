...
module Make (Theory : Theory.S) = struct
  ...
  module Induct : sig
    ...
    let fake_sub_ps_unsafe n =
      ...
      let bP k =
        Display_maps.var_apply_sub (Cone.base k) (Cone.bdry_right n k)
      in
      let f k =
        Display_maps.var_apply_sub (Cone.filler k) (Cone.bdry_left n k)
      in
      let fP k =
  ...
  module Codim1 = struct
    ...
    let right_filler n =
      Display_maps.var_apply_sub (Cone.filler n) (right_incl n)
