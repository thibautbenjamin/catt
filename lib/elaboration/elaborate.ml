...
module Make (Environment : Environments.S) = struct
  module Translate_raw = Translate_raw.Make (Environment)
  module Raw = Raw.Make (Environment)
  open Environment
  open K
  ...
  module Constraints = struct
    ...
    let substitute_ty l ty =
      let ty =
        List.fold_left
          (fun ty (i, tm) -> ty_replace_meta_tm (i, tm) ty)
          ty l.utm
      in
      List.fold_left
        (fun ty (i, ty') -> ty_replace_meta_ty (i, ty') ty)
        ty l.uty

    let substitute_tm l tm =
      List.fold_left
        (fun tm (i, tm') -> tm_replace_meta_tm (i, tm') tm)
        tm l.utm
  ...
  module Constraints_typing = struct
    ...
    and sub src meta_ctx s tgt cst =
      Io.info ~v:5
        (lazy
          (Printf.sprintf
             "constraint typing substitution %s in ctx %s, target %s, meta_ctx \
              %s"
  ...
  let solve_cst ~elab_fn ~print_fn ~kind x =
    ...
    try
      let x = elab_fn x in
      Io.info ~v:3
        (lazy (Printf.sprintf "%s elaborated to %s" kind (print_fn x)));
      x
  ...
  let ty_in_ps ps t =
    try
      ...
      with
      | InvalidPS -> raise (Error.invalid_ps (Printing.ctx_to_string ps))
