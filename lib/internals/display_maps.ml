...
struct
  ...
  struct
    ...
    let var_apply_sub v s =
      ...
      | Var v -> v
      | _ ->
          Error.fatal "image of a variable by a display map must be a variable"
    ...
    let rec glue s1 s2 inr c2 dm =
      ...
      | _, [], _ :: _ ->
          Error.fatal
            "wrong data for pullback gluing: display map cannot be longer than \
             the context"
      | _, _, _ ->
          Error.fatal
