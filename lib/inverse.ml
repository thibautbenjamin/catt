...
= struct
  ...
  let coh c =
    if (not (Kernel.is_inv_coh c)) then raise CohNonInv;
    let ps,ty,(name,susp,func) = Kernel.forget_coh c in
    let ty_inv = match ty with
      | Obj | Meta_ty _ -> assert false
      | Arr (a,u,v) -> Arr (a,v,u)
    in
    Kernel.check_coh ps ty_inv (name^"^-1",susp,func)
  ...
  let group_vertically ps t src_t tgt_t =
    let
      coh_unbiased =
      Kernel.check_noninv_coh ps src_t tgt_t ("unbiased_comp",0,None)
    in
    ...
    Kernel.check_inv_coh ps t t_vertically_grouped ("vertical_grouping",0,None)

  type lin_comp = {arity: int; dim: int;  sub_ps: sub_ps}

  let tm_to_lin_comp t =
    let ps,sub_ps = match t with
      | Coh (c,s) -> let ps,_,_  = Kernel.forget_coh c in ps, s
  ...
  and compute_witness_coh_inv c s ~ps ~pp_data ~d ~sub_base ~u ~v =
    ...
    in
    let
      c_wit = Kernel.check_inv_coh ps src_wit tgt_wit  (name^"_Unit",susp,func)
    in
    Coh(c_wit,s)
  ...
  and compute_witness_comp c s ~ps ~d ~sub_base ~u ~v =
    ...
    let t =
      ...
      let w = match Kernel.forget_coh c_op with
        | _,Arr(_,_,v),_ ->
    ...
    in
    let src_c,_ = Kernel.coh_noninv_srctgt c in
