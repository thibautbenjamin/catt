...
and right_base n m k =
  ...
  | _, 1 -> (
      let _, right_incl = ctx_comp_codim1 m in
      match Unchecked.tm_apply_sub (Var (base m)) right_incl with
      | Var x -> x
      | _ -> assert false)
...
and right_filler n m k =
  ...
  | _, 1 -> (
      let _, right_incl = ctx_comp_codim1 m in
      match Unchecked.tm_apply_sub (Var (filler m)) right_incl with
      | Var x -> x
      | _ -> assert false)
...
let compose_base () =
  ...
  let ctx, right_incl = ctx_comp_codim1 2 in
  let left_filler = with_type ctx (left_filler 2) in
  let right_filler = with_type ctx (right_filler 2 2 1) in
  let left_base = with_type ctx (left_base 2) in
  let right_base = with_type ctx (right_base 2 2 1) in
  let tm_1 =
