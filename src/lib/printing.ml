...
struct
  ...
  struct
    ...
    module Kolmogorov = struct
      ...
      let seen t decls =
        List.exists (is_equal_value t) decls
      ...
      and collect_decls_tm decls = function
        ...
        | Coh (c, s) ->
            let decls =
              if seen (Coh c) decls then decls
              else
                (let decls = Coh c :: decls in
                 collect_decls_coh decls c)
            in
            collect_decls_sub_ps decls s
        | App (t, s) ->
            let decls =
              if seen (Tm t) decls then decls
              else
                (let decls = Tm t :: decls in
      ...
      let order_decls decls =
        ...
        let rec add_next decls ordered front =
          match decls with
          | [] -> assert false
          | t :: decls when all_deps_done t ordered ->
              let n = new_name () in
              (List.append front decls, (t, n) :: ordered)
          | t :: decls -> add_next decls ordered (t :: front)
        in
